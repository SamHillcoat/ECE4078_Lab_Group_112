...
class EKF:
    ...
    def recover_from_pause(self, measurements):
        if not measurements:
            print('ekf recover: no measurements')
            return False
        else:
            lm_new = np.zeros((2,0))
    ...
    def add_landmarks(self, measurements,true_pos=False):
        ...
        for lm in measurements:
            ...
            lm_bff = lm.position #ROBOT FRAME  

            if true_pos: #lm.postion is already in the world frame (from map)
                lm_inertial = lm.position
            else:
                lm_inertial = robot_xy + R_theta @ lm_bff # WORLD FRAME

            self.taglist.append(int(lm.tag))
            self.markers = np.concatenate((self.markers, lm_inertial.reshape(2,1)), axis=1)
            #print(self.markers.shape[1])
