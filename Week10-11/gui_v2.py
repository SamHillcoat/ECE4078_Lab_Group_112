...
class Game:
    ...
    def relative_point(self, start, target, num_points=50):
        '''
        Determine a relative point near the target fruit to navigate to

        Returns minimum distance path out of the 50 generated points
        '''
        fruit_radius = 0.5
        points = []
        ...
        while len(points) <= num_points:
            # generate random point within the radius of the target
            r = fruit_radius * math.sqrt(np.random.random())
            theta = np.random.random() * 2 * math.pi

            x = target[0] + r * math.cos(theta)
            y = target[0] + r * math.sin(theta)

            collision = False

            # if point is in collision with an obstacle, discard it
            for obs in self.all_obstacles:
                if obs.is_in_collision_with_points([(x, y)]):
                    collision = True
                    break
        ...
        for end in points:
            path, dist = self.generate_path(start, end)
            if dist < min_dist:
                min_path = path
